    ...
    if (averages == NULL)
    {
      ...
      exit(1);
		...
		while(!feof(data))		//continue until end of file
		{
			...
			++lineno;
			//parse each line
     while(nfields == (nret = fscanf(data, "%s %s %s %2[^.].%2[^.].%d %4[^,],%s", title, time, region, date.day, date.month, &date.year, temp_main, temp_sub))) {
        ...
        if(strcmp(current_month, date.month) != 0) {   //when the month changes
          ...
          fprintf(averages, "Average Temperature: %.2lf\n", avg_temp);
